...
CREATE TABLE smart_sensors (
    ...
    type VARCHAR(20) NOT NULL CHECK (type IN ('light', 'thermostat')),
    state VARCHAR(10) NOT NULL CHECK (state IN ('on', 'off')),
    date_added DATE NOT NULL DEFAULT CURRENT_DATE);

-- Set a starting value for sensor_id of 1000
ALTER SEQUENCE smart_sensors_id_seq RESTART WITH 1000;

-- Insert sensor sample entries
INSERT INTO smart_sensors (location, type, state, date_added) VALUES
